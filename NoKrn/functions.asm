...
keyboard:
    ...
    .keyboard:
        ...
        cmp al, 0x01; ctrlA
        JE fillScreen ; jump if true?

        cmp al, 0x07; ctrlG
        JE fillScreen ; jump if true?

        cmp al, 0x12; ctrlR
        JE Reboot ; jump if true?
        int 0x10
...
printMem:
    ...
    .print_loop:
        ...
        lodsb             ; Load the byte at SI into AL and advance SI (Why the hell is this an instruction)
...
fillScreen:
    ; Store memory location to be read in si and store bytes to be read after that in cx
    mov cx, 1999 ; How many charicters to print to the screen
    .loop:
        mov ah, 0x0E      ; AH = 0Eh (Teletype output)
        mov al, 0x23      ; Print # charicter
        int 0x10          ; BIOS interrupt for screen output
        loop .loop        ; Continue printing until CX becomes zero
    ret
    hlt
